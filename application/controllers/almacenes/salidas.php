...
class Salidas extends CI_Controller {
    ...
    public function ordenes_salida($offset = 0) {
        ...
        foreach ($datos as $d) {
            $this->table->add_row(
                    $d->ruta, $d->ordenes, array('data' => number_format($d->piezas, 2), 'style' => 'text-align: right;'), array('data' => number_format($d->peso, 2), 'style' => 'text-align: right;'), array('data' => anchor($this->folder . $this->clase . 'ordenes_salida_ruta/' . $d->id_ruta, '<i class="icon-list"></i>', array('class' => 'btn btn-small', 'title' => 'Ordenes de salida')), 'style' => 'text-align: right;')
            );
        }
        $data['table'] = $this->table->generate();
    ...
    public function ordenes_salida_procesadas($offset = 0) {
        ...
        foreach ($datos as $d) {
            ...
            //$piezas = $this->os->get_piezas($d->id);
            $this->table->add_row(
                    $d->ruta, $d->ordenes, array('data' => number_format($d->piezas, 2), 'style' => 'text-align: right;'), array('data' => number_format($d->peso, 2), 'style' => 'text-align: right;'), array('data' => anchor($this->folder . $this->clase . 'ordenes_salida_ruta_procesadas/' . $d->id_ruta, '<i class="icon-list"></i>', array('class' => 'btn btn-small', 'title' => 'Ordenes de salida')), 'style' => 'text-align: right;')
            );
        }
        $data['table'] = $this->table->generate();
