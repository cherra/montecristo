...
class Salidas extends CI_Controller {
    ...
    public function ordenes_salida($offset = 0){
    	...
    	$datos = $this->os->get_grouped_by_ruta($page_limit, $offset, $filtro, array('1','2'))->result();
    	
        // generar paginacion
    	$this->load->library('pagination');
    	$config['base_url'] = site_url($this->folder.$this->clase.'index');
    	$config['total_rows'] = $this->os->count_grouped_by_ruta($filtro, array('1','2'));
    ...
    public function ordenes_salida_ruta( $id_ruta = NULL, $offset = 0){
    	...
    	foreach ($datos as $d) {
    		...
    		$this->table->add_row(
                        ...
                        $sucursal->poblacion.', '.$sucursal->municipio,
