...
/**
 * Description of Numero_letras
 *
 * @author cherra
 */
class Numero_letras {
    
    private function unidad($numuero){  //UNIDADES
        switch ($numuero)
        {
            case 9:
            {
   ...
   private function decena($numdero){              //DECENAS
    ...
    private function centena($numc){       //CENTENAS
     if ($numc >= 100)
     {
    ...
    private function miles($nummero){                  //MILES
    ...
    private function cienmiles($numcmero){                  //CIENMILES
     if ($numcmero == 100000)
   ...
   function convertir($numero){
        ...
        $arreglo = explode('.',$decimal);
        $fraccion = $arreglo[1];
        //$decimales = $numero-$entero;
        $numero = sprintf("%u",$numero);
        $numf = $this->milmillon($numero).' PESOS '.$fraccion.'/100 M.N.';;
        //$numf = $this->milmillon($numero).' PESOS '.$decimales;
        if($negativo == TRUE)
            $numf = "MENOS ".$numf;
        return $numf;
   }
