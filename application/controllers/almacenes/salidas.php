...
class Salidas extends CI_Controller {
    ...
    public function ordenes_salida_proceso_ruta($id_ruta = NULL, $offset = 0) {
        ...
        $this->load->view('almacenes/salidas/ruta_lista', $data);
    }
    ...
    public function ordenes_salida_procesadas($offset = 0) {
        ...
        foreach ($datos as $d) {
            $this->table->add_row(
                    ...
                    array('data' => number_format($d->piezas, 2), 'style' => 'text-align: right;'), 
                    array('data' => number_format($d->peso, 2), 'style' => 'text-align: right;'),
                    array('data' => anchor($this->folder . $this->clase . 'ordenes_salida_procesadas_ruta/' . $d->id_ruta, '<i class="icon-list"></i>', array('class' => 'btn btn-small', 'title' => 'Ordenes de salida')), 'style' => 'text-align: right;')
            );
        }
        $data['table'] = $this->table->generate();
    ...
    public function ordenes_salida_procesadas_ruta($id_ruta = NULL, $offset = 0) {
        $this->load->model('orden_salida', 'os');
        $this->load->model('cliente', 'c');
        $this->load->model('sucursal', 's');
        $this->load->model('almacen', 'a');
        ...
        //$data['link_add'] = anchor($this->folder.$this->clase.'pedidos_agregar','<i class="icon-plus icon-white"></i> Nuevo', array('class' => 'btn btn-inverse'));
        $data['link_back'] = anchor($this->folder . $this->clase . 'ordenes_salida_procesadas', '<i class="icon-arrow-left"></i> Regresar', array('class' => 'btn'));
        $data['action'] = $this->folder . $this->clase . 'ordenes_salida_procesadas_ruta/' . $id_ruta;
        ...
        if ($this->input->post()) {
            ...
            foreach ($salidas as $s) {
                $this->os->update($s, array('estado' => '3', 'id_almacen' => $id_almacen, 'fecha_entrega' => $fecha)); // Estado 3 = Enviado
        ...
        // generar paginacion
        $this->load->library('pagination');
        $config['base_url'] = site_url($this->folder . $this->clase . 'ordenes_salida_procesadas_ruta');
    ...
    public function ordenes_salida_enviadas($offset = 0) {
        ...
        $page_limit = $this->config->item("per_page");
        $datos = $this->os->get_by_estado(array('3'), $page_limit, $offset, $filtro)->result();  // Estado 3 = Enviadas

        // generar paginacion
        $this->load->library('pagination');
        $config['base_url'] = site_url($this->folder . $this->clase . 'ordenes_salida_enviadas');
        $config['total_rows'] = $this->os->count_by_estado(array('3'), $filtro);
        $config['per_page'] = $page_limit;
        $config['uri_segment'] = 4;
        $this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
